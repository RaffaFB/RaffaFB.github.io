...
function drawClock(place) {
  ...
  drawClock.romans.reverse().map((n, i) => { // invert numbers
    context.fillStyle = n.c;
    var coord = polar2Cartesian(0.85 * clockRadius, (i+1) * fiveMin); // adjust i to 1
  ...
  drawClock.decimals.reverse().map((n, i) => { // reverse
    context.fillStyle = n.c;
    // runs at half the speed
    var coord = polar2Cartesian(1.01 * clockRadius, (i+1) * fiveMin * 0.5); // adjust i to 1
    // translate to the center of the canvas
    coord = translate(coord, center);
    context.fillText(n.txt, coord.x, coord.y);
  });
...
var runAnimation = (() => {
  ...
  function drawHandles() {
    ...
    // counterclockwise handle moviment
